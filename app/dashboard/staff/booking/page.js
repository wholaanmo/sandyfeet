// app/dashboard/staff/booking/page.js
'use client';
...
export default function StaffBooking() {
  ...
  const ROOMS_CHECKOUT_CACHE_KEY = 'staffBookingDraft';
  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(q, (querySnapshot) => {
      ...
      try {
        const savedState = sessionStorage.getItem(ROOMS_CHECKOUT_CACHE_KEY);
        if (savedState) {
 ...
 const handleProceed = () => {
  ...
  if (isExclusiveResortBooking && !canBookExclusiveResort) {
    setDateSelectionError('Cannot proceed because some rooms are unavailable on the selected date.');
    return;
  }
  ...
  // Save to session storage and navigate to booking process
  sessionStorage.setItem('staffMultiRoomBooking', JSON.stringify(bookingData));
  // UPDATED: Navigate to book-process with room tab and process flag
  router.push('/dashboard/staff/book-process?tab=room&process=true');
};
        ...
        <div className="lg:col-span-8">
          {availableRoomTypes.length === 0 ? (
            <div className="bg-white rounded-2xl border border-gray-200 shadow-sm p-12 text-center">
              ...
              <h3 className="text-xl font-playfair font-semibold text-gray-900 mb-2">No Rooms Available</h3>
              <p className="text-gray-600 text-sm max-w-xs mx-auto">All rooms are currently booked. Check back later for availability.</p>
            </div>
          ) : (
                        ...
                        {quantity === 0 ? (
                          <button
                            ...
                            className={`w-full py-3 text-sm font-semibold rounded-xl transition-all flex items-center justify-center gap-2 ${isTentDisabled
                              ...
                              : realTimeAvailable > 0
                                ? 'bg-[#4D8CF5] hover:bg-[#3B78E7] text-white shadow-[0_4px_14px_0_rgb(37,99,235,0.39)] hover:shadow-[0_6px_20px_rgb(37,99,235,0.23)] hover:-translate-y-0.5'
                          ...
                          <div className="w-full flex items-center justify-between bg-blue-50/80 border border-blue-200 rounded-xl p-1.5 shadow-inner">
                            <button
                              onClick={() => handleQuantityChange(room.type, false)}
                              disabled={isTentDisabled}
                              className={`w-10 h-10 flex items-center justify-center rounded-lg shadow-sm transition-colors ${isTentDisabled
                            ...
                            <button
                              ...
                              className={`w-10 h-10 flex items-center justify-center text-white rounded-xl shadow-sm transition-colors ${isTentDisabled
                                ? 'bg-gray-400 cursor-not-allowed'
                                : 'bg-[#4D8CF5] hover:bg-[#3B78E7]'
                              }`}
                            >
          ...
          <div className="bg-white rounded-[2rem] border border-gray-100 shadow-[0_12px_40px_rgb(0,0,0,0.06)] p-6">
              ...
              <div className="flex items-start justify-between gap-3">
                ...
                <button
                  ...
                  className={`rounded-xl px-3 py-2 text-xs font-bold transition-colors ${isExclusiveResortBooking
                    ...
                    : 'bg-[#4D8CF5] text-white hover:bg-[#3B78E7]'
                  }`}
                >
              ...
              {isModalOpen && (
                <div ref={calendarPopoverRef} className="absolute left-0 top-[4.25rem] z-50 bg-white w-[290px] max-w-[calc(100vw-3rem)] rounded-2xl shadow-[0_16px_40px_rgb(0,0,0,0.14)] p-2.5 border border-gray-100 max-h-[58vh] overflow-hidden flex flex-col">
                  ...
                  <div className="pt-1.5 mt-1.5 border-t border-gray-100 bg-white">
                    <button type="button" onClick={() => setIsModalOpen(false)} className="w-full py-2 bg-[#4D8CF5] text-white rounded-xl text-xs font-bold hover:bg-[#3B78E7] transition-all shadow-lg shadow-blue-500/20">
                      Confirm Dates
                    </button>
                  </div>
                </div>
            ...
            <div className="relative group">
              <button
                ...
                className={`w-full py-4 font-bold rounded-xl transition-all duration-300 flex items-center justify-center gap-2 ${((checkInDate && (isExclusiveResortBooking || Object.values(selectedRooms).some(q => q > 0))) && !hasAnyGuestErrors)
                  ? 'bg-[#4D8CF5] text-white shadow-[0_8px_20px_rgb(37,99,235,0.25)] hover:bg-[#3B78E7] hover:-translate-y-0.5'
                  : 'bg-gray-100 text-gray-400 cursor-not-allowed border border-gray-200'
                }`}
              >
                Book Now &bull; ₱{checkoutTotal.toLocaleString()} &rarr;
              </button>
              {(!checkInDate || (!isExclusiveResortBooking && Object.values(selectedRooms).every(q => q === 0))) && (
                <div className="absolute left-1/2 -top-10 -translate-x-1/2 opacity-0 group-hover:opacity-100 transition-opacity bg-gray-800 text-white text-[10px] font-medium py-1.5 px-3 rounded-lg shadow-xl whitespace-nowrap pointer-events-none z-10">
              ...
              {hasAnyGuestErrors && (checkInDate && (isExclusiveResortBooking || Object.values(selectedRooms).some(q => q > 0))) && (
                <div className="absolute left-1/2 -top-10 -translate-x-1/2 opacity-0 group-hover:opacity-100 transition-opacity bg-red-600 text-white text-[10px] font-medium py-1.5 px-3 rounded-lg shadow-xl whitespace-nowrap pointer-events-none z-10">
                  Please fix guest count errors before proceeding.
          ...
          >
            <div className="flex items-center justify-between border-b border-slate-200 bg-white px-6 py-4 flex-none sticky top-0 z-10">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h3 className="text-xl font-bold tracking-tight text-slate-900">Exclusive Resort Booking</h3>
                </div>
              </div>
              <button
                type="button"
