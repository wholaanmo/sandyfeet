...
export const getTypeDisplay = (b) => {
  ...
  // Multi‑room only if there are multiple distinct room types
  if (b.isMultiRoom && b.roomTypesArray && b.roomTypesArray.length > 1) {
    return { label: 'Multi-Room Types', color: 'bg-violet-100 text-violet-700', icon: 'fa-layer-group' };
  }
  // Single room type (one distinct type, any quantity) → display "Single Room Type"
  return { label: 'Single Room Type', color: 'bg-blue-100 text-blue-700', icon: 'fa-bed' };
};
...
export const getBookingTitle = (b) => {
  ...
  // Multi‑room with different types → show actual room type names
  if (b.isMultiRoom && b.roomTypesArray && b.roomTypesArray.length > 1) {
    const roomNames = b.roomTypesArray.map(r => r.type).join(', ');
    return roomNames;
  }
  // Single room type → show actual room type name (e.g., "Tent", "Cottage", "Cabin")
  const roomTypeName = b.roomType || (b.roomTypesArray?.[0]?.type) || 'Room';
  return roomTypeName;
};
...
export const getGuestTotal = (b) => {
  ...
  // For grouped bookings (multi-room or single-room-type with multiple units)
  if (b.children && b.children.length > 0) {
    // Use the aggregated totalGuests computed during grouping
    return b.totalGuests || b.children.reduce((sum, child) => sum + (child.guests || 0), 0);
  }
  // Fallback for single-unit bookings
  return b.guests || 0;
};
...
export const canCancel = (b) => {
  if (['cancelled', 'cancelled-by-guest', 'completed'].includes(b.status)) return false;
...
export const lookupByReference = async (email, refNumber) => {
  ...
  if (!childSnap.empty) {
    ...
    const roomTypesArray = Object.entries(roomTypes).map(([t, d]) => ({ type: t, quantity: d.quantity, guestsPerRoom: d.guestsPerRoom, price: d.price }));
    const isMultiRoom = roomTypesArray.length > 1;
    return {
      ...
      isMultiRoom, isMultiRoomBooking: true, totalRooms, totalGuests,
...
export const cancelBooking = async (booking, reason) => {
  ...
  try {
    ...
    if (booking.isMultiRoom && booking.children) {
      ...
      const distinctTypes = booking.roomTypesArray?.length || 0;
      if (booking.isExclusiveResortBooking) {
        notif.roomType = 'Entire Resort';
      } else if (distinctTypes > 1) {
        notif.roomType = 'Multi-Room Types';
      } else {
        notif.roomType = 'Single Room Type';
      }
    } else if (booking.type === 'daytour') {
      ...
      notif.roomType = booking.isExclusiveResortBooking ? 'Entire Resort' : (booking.roomType || 'Single Room Type');
    }
  ...
  try {
    ...
    if (booking.type === 'daytour') {
      ...
      let roomStr;
      if (booking.isExclusiveResortBooking) {
        roomStr = 'Entire Resort';
      } else if (booking.isMultiRoom && booking.roomTypesArray && booking.roomTypesArray.length > 1) {
        roomStr = (booking.roomTypesArray || []).map(r => `${r.quantity} x ${r.type}`).join(', ');
      } else {
        const roomType = booking.roomType || (booking.roomTypesArray?.[0]?.type) || 'Room';
