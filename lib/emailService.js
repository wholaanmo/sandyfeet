...
export async function sendConfirmationEmail(booking, adminNote = '') {
  ...
  // ✅ Get checkinToken – works for all booking types
  let checkinToken = booking.checkinToken;
  if (!checkinToken && booking.isMultiRoomGroup && booking.originalChildBookings?.length) {
    // Take token from first child booking
    checkinToken = booking.originalChildBookings[0].checkinToken;
  }
  const qrCodeUrl = checkinToken
    ? `${resolveBaseUrl()}/check-in?token=${checkinToken}`
    : null;
  ...
  const emailContent = `
    <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 40px 20px; background-color: #ffffff; color: #333333; line-height: 1.6; border: 1px solid #eeeeee;">
      ...
      <div style="text-align: center; margin: 30px 0;">
        <a href="${trackerUrl}" style="background-color: #111111; color: #ffffff; padding: 7px 20px; text-decoration: none; border-radius: 6px; font-size: 12px; font-weight: bold; display: inline-block;">Manage Reservation</a>
      </div>
      ` : ''}

      ${qrCodeUrl ? `
      <!-- QR CODE SECTION -->
      <div style="text-align: center; margin: 30px 0; padding: 20px; background-color: #f0f9ff; border-radius: 12px; border: 1px solid #bae6fd;">
        <h3 style="margin: 0 0 15px 0; font-size: 16px; font-weight: bold; color: #0369a1;">Check-in QR Code</h3>
  ...
  <a 
    href="${resolveBaseUrl()}/api/download-qr?token=${checkinToken}" 
    style="display: inline-block; background-color: #0369a1; color: #ffffff; padding: 6px 16px; text-decoration: none; border-radius: 6px; font-size: 12px; font-weight: bold;"
  >
    ⬇ Download QR Code
  </a>
</div>
        <p style="margin: 15px 0 0 0; font-size: 12px; color: #475569;">
          Please present this QR code to staff upon check-in. You can also save or screenshot this QR code for easy access.
        </p>
      </div>
      ` : ''}
  ...
  const emailContent = `
    <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 40px 20px; background-color: #ffffff; color: #333333; line-height: 1.6; border: 1px solid #eeeeee;">
      ...
      <div style="background-color: #f9fafb; padding: 15px; border-radius: 8px; margin: 20px 0; border: 1px solid #f3f4f6;">
        ...
        ${cancelledBy === 'guest' ? `
          <p style="margin: 10px 0 0; font-size: 13px; color: #991b1b;"><strong>Refund Policy:</strong> The full down payment will be retained by the resort in accordance with our cancellation policy.</p>
        ` : ''}
      </div>
...
export async function sendPasswordResetEmail(to, firstName, resetLink) {
  const emailHtml = `
    <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 40px 20px; background-color: #ffffff; color: #333333; line-height: 1.6; border: 1px solid #eeeeee;">
      <div style="text-align: center; margin-bottom: 30px;">
        <h1 style="margin: 0; font-size: 24px; font-weight: bold; color: #111111;">Reset Your Password</h1>
        <p style="margin: 5px 0 0; font-size: 14px; color: #3b82f6;">SandyFeet Guest Account</p>
      </div>
      <p>Dear <strong>${firstName || 'Guest'}</strong>,</p>
      <p>We received a request to reset the password for your SandyFeet account.</p>
      <p>Click the button below to set a new password. This link will expire in 15 minutes.</p>
      <div style="text-align: center; margin: 30px 0;">
        <a href="${resetLink}" style="background-color: #111111; color: #ffffff; padding: 10px 24px; text-decoration: none; border-radius: 6px; font-size: 14px; font-weight: bold;">Reset Password</a>
      </div>
  ...
  return await sendEmail(to, 'Reset Your SandyFeet Account Password', emailHtml);
}
...
export async function sendMoveDateNotificationEmail(booking, adminMessage = '') {
  ...
  <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 40px 20px; background-color: #ffffff; color: #333333; line-height: 1.6; border: 1px solid #eeeeee;">
    ...
    <div style="text-align: center; margin-bottom: 30px;">
      <h1 style="margin: 0; font-size: 24px; font-weight: bold; color: #111111;">Refund Notice</h1>
      <p style="margin: 5px 0 0; font-size: 14px; color: #3b82f6; text-transform: uppercase; letter-spacing: 1px;">Reservation Refund Update</p>
    </div>

    <p>Dear <strong>${guestName}</strong>,</p>

    <p>
      We would like to inform you that your reservation request has been reviewed by the resort management.
      Your down payment refund is currently being processed.
    </p>

    <div style="border-top: 1px solid #eeeeee; border-bottom: 1px solid #eeeeee; padding: 20px 0; margin: 25px 0;">
      <h2 style="margin: 0 0 15px; font-size: 14px; font-weight: bold; color: #999999; text-transform: uppercase;">
        Reservation Details
      </h2>

      <p style="margin: 5px 0; font-size: 14px;">
        <span style="color: #999999; width: 140px; display: inline-block;">Booking ID:</span>
        <strong>${bookingId}</strong>
      </p>

      ${isDayTour ? `
        <p style="margin: 5px 0; font-size: 14px;">
          <span style="color: #999999; width: 140px; display: inline-block;">Tour Date:</span>
          <strong>${formatDateOnly(booking.selectedDate)}</strong>
        </p>
      ` : `
        <p style="margin: 5px 0; font-size: 14px;">
          <span style="color: #999999; width: 140px; display: inline-block;">Check-in:</span>
          <strong>${originalCheckIn}</strong>
        </p>

        <p style="margin: 5px 0; font-size: 14px;">
          <span style="color: #999999; width: 140px; display: inline-block;">Check-out:</span>
          <strong>${originalCheckOut}</strong>
        </p>
      `}
    </div>
    ...
    <div style="background-color: #f9fafb; padding: 15px; border-radius: 8px; margin: 20px 0; border: 1px solid #f3f4f6;">
      <p style="margin: 0; font-size: 14px; color: #4b5563;">
        <strong>Message from Resort:</strong><br />
    ...
    <p style="font-size: 14px; color: #4b5563;">
      If you have any questions regarding your refund, please feel free to contact the resort directly.
    </p>

    <div style="text-align: center; margin: 30px 0;">
      <a href="${trackerUrl}" style="background-color: #111111; color: #ffffff; padding: 7px 20px; text-decoration: none; border-radius: 6px; font-size: 12px; font-weight: bold; display: inline-block;">
        Manage Reservation
      </a>
    </div>
...
`;
  ...
  return await sendEmail(guestEmail, `Down Payment Refund - ${bookingId}`, emailContent);
}
...
export async function sendIdRequestEmail(booking, adminMessage = '') {
  ...
  const emailContent = `
    <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 40px 20px; background-color: #ffffff; color: #333333; line-height: 1.6; border: 1px solid #eeeeee;">
      ...
      <p style="font-size: 14px;">Please upload a clear photo of your ID in your SandyFeet Reservation website account. Thank you for your cooperation.</p>
...
export async function sendPaymentRequestEmail(booking, adminMessage = '') {
  const guestEmail = booking.guestInfo?.email;
  const guestName = `${booking.guestInfo?.firstName} ${booking.guestInfo?.lastName}`;
  const bookingId = booking.bookingId;
  ...
  const emailContent = `
    <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 40px 20px; background-color: #ffffff; color: #333333; line-height: 1.6; border: 1px solid #eeeeee;">
      <div style="text-align: center; margin-bottom: 30px;">
        <h1 style="margin: 0; font-size: 24px; font-weight: bold; color: #111111;">Payment Request</h1>
        <p style="margin: 5px 0 0; font-size: 14px; color: #3b82f6; text-transform: uppercase; letter-spacing: 1px;">Action Required</p>
      </div>

      <p>Dear <strong>${guestName}</strong>,</p>
      <p>To finalize your reservation at SandyFeet Resort, we kindly request an updated copy of your payment proof.</p>
      ...
      <p style="font-size: 14px;">Please upload a clear photo of your payment proof in your SandyFeet Reservation website account. Thank you for your cooperation.</p>
  ...
  return await sendEmail(guestEmail, `Payment Request - ${bookingId}`, emailContent);
}
...
export async function sendDayTourPendingEmail(booking) {
  ...
  const emailContent = `
    <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 40px 20px; background-color: #ffffff; color: #333333; line-height: 1.6; border: 1px solid #eeeeee;">
      ...
      <div style="background-color: #f9fafb; padding: 15px; border-radius: 8px; margin: 20px 0; border: 1px solid #f3f4f6;">
        <p style="margin: 0; font-size: 14px; color: #4b5563;">You will receive a separate confirmation email once the resort admin confirms your reservation. You can also track your reservation through your account.</p>
      </div>

      <div style="text-align: center; margin: 30px 0;">
        <a href="${trackerUrl}" style="background-color: #111111; color: #ffffff; padding: 7px 20px; text-decoration: none; border-radius: 6px; font-size: 12px; font-weight: bold; display: inline-block;">Track Reservation</a>
      </div>
...
export async function sendDayTourConfirmationEmail(booking, adminNote = '') {
  ...
  // Get checkinToken – day tours store it directly in the booking document
  const checkinToken = booking.checkinToken;
  const qrCodeUrl = checkinToken
    ? `${resolveBaseUrl()}/check-in?token=${checkinToken}`
    : null;
  ...
  const emailContent = `
    <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 40px 20px; background-color: #ffffff; color: #333333; line-height: 1.6; border: 1px solid #eeeeee;">
      ...
      <div style="border-top: 1px solid #eeeeee; border-bottom: 1px solid #eeeeee; padding: 20px 0; margin: 25px 0;">
        ...
        <div style="background-color: #f9fafb; padding: 15px; border-radius: 8px; margin: 20px 0; border: 1px solid #f3f4f6;">
        <p style="margin: 0; font-size: 14px; color: #4b5563;">You will receive a separate confirmation email once the resort admin confirms your reservation. You can track your reservation through your account.</p>
      </div>
