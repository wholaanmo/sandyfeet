...
/** Philippine calendar date key (Asia/Manila). */
export const toLocalDateKey = (date) => {
  if (!date) return '';
  if (date instanceof Date) return getPhilippineDateKeyFromInstant(date);
...
export const aggregateRoomAvailabilityFromBookings = (bookingDocs = []) => {
  ...
  bookingDocs.forEach((booking) => {
    ...
    const checkInKey = getPhilippineDateKeyFromInstant(booking.checkIn);
    const checkOutKey = getPhilippineDateKeyFromInstant(booking.checkOut);
    const roomId = booking.roomId;
    if (!checkInKey || !checkOutKey || checkOutKey <= checkInKey || !roomId) return;

    const numberOfRooms = Math.max(1, Number(booking.numberOfRooms) || 1);
    const tentCount = Math.max(0, Number(booking.tentCount) || 0);

    for (const dateKey of iteratePhilippineDateKeysInRange(checkInKey, checkOutKey)) {
...
export const normalizeDayTourDateKey = (selectedDate) => {
  ...
  return getPhilippineDateKeyFromInstant(selectedDate) || String(selectedDate);
};
