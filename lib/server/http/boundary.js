 ...
 * Uses the server-authoritative session cookie ONLY.
 * NEVER trusts client-supplied headers for authentication in production.
 *
...
async function resolveActor(request) {
  try {
    ...
    return await resolveSession(sessionValue);
  } catch {
    return null;
  }
}
