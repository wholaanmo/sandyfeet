...
const getBookingTotalPrice = (booking = {}) =>
  Number(booking.totalPrice || booking.manualTotalPrice || 0);

const getBookingDownPaymentAmount = (booking = {}) => {
  const explicit = Number(booking.downPayment);
  if (!Number.isNaN(explicit) && explicit > 0) return explicit;
  return getBookingTotalPrice(booking) * 0.5;
};

/** Remaining down payment for a submitted booking (₱0 when proof uploaded or down payment satisfied). */
export const getRemainingDownPaymentDue = (booking = {}) => {
  if (hasUploadedPaymentProof(booking)) return 0;

  const downPayment = getBookingDownPaymentAmount(booking);

  if (CONFIRMED_BOOKING_STATUSES.includes(booking.status)) {
    const adminBalance = booking.balance ?? booking.manualBalance;
    if (adminBalance != null && adminBalance !== '') {
      const numeric = Number(adminBalance);
      if (!Number.isNaN(numeric)) {
        const paidTowardTotal = Math.max(0, getBookingTotalPrice(booking) - Math.max(0, numeric));
        return Math.max(0, downPayment - paidTowardTotal);
      }
    }
    return 0;
  }
...
/** Down payment due shown in Pending Payment (bank request before submit; booking fields after). */
export const getDownPaymentDueForBankRequest = (bankRequest, bookings = []) => {
  const related = getRelatedBookings(bankRequest, bookings);

  if (related.some(hasUploadedPaymentProof)) return 0;

  if (related.length > 0) {
    return related.reduce((sum, booking) => sum + getRemainingDownPaymentDue(booking), 0);
  }

  return Number(
    bankRequest?.downPayment ||
      bankRequest?.downPaymentRequired ||
      bankRequest?.totalPrice ||
      bankRequest?.totalAmount ||
      0
  );
};
...
export const isPendingBankPaymentRequest = (bankRequest, bookings = []) => {
  ...
  if (related.some(hasUploadedPaymentProof)) return false;

  if (isBookingConfirmedForBankRequest(bankRequest, bookings)) return false;

  if (related.length > 0 && getDownPaymentDueForBankRequest(bankRequest, bookings) === 0) {
    return false;
  }
...
export const getPendingPaymentBookingDetails = (bankRequest, bookings = []) => {
  ...
  details.push({
    ...
    value: `₱${getDownPaymentDueForBankRequest(bankRequest, bookings).toLocaleString()}`,
  });
