...
// Function to fetch and determine room type for a booking (for cancellation notifications)
const getCancellationRoomTypeDisplay = async (bookingId, bookingType) => {
  try {
    if (bookingType === 'room') {
      // First check if this is a multi-room booking by looking for children
      const bookingsRef = collection(db, 'bookings');
      const childQuery = query(bookingsRef, where('parentBookingId', '==', bookingId), where('isMultiRoomBooking', '==', true));
      const childSnapshot = await getDocs(childQuery);
      
      if (!childSnapshot.empty) {
        // This is a multi-room booking
      ...
      // Check if it's a single booking
      const bookingRef = doc(db, 'bookings', bookingId);
      const bookingSnap = await getDocs(query(collection(db, 'bookings'), where('bookingId', '==', bookingId)));
    ...
    console.error('Error determining room type for cancellation:', error);
    return 'Single Room Type';
  }
};
