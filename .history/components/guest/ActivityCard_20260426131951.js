...
export default function ActivityCard({ activity }) {
  ...
  // Get all non-empty images from the activity
  const images = useMemo(() => {
    const allImages = Array.isArray(activity.images) ? activity.images.filter(Boolean) : [];
    return allImages;
  }, [activity.images]);
                ...
                <div className="relative min-h-[320px] bg-gradient-to-br from-ocean-pale/60 to-ocean-ice/70 sm:min-h-[360px]">
                  ...
                  {images.length > 1 && (
                    <button
                      ...
                      className="absolute left-3 top-1/2 -translate-y-1/2 rounded-full bg-black/45 px-3 py-2 text-white transition-colors duration-200 hover:bg-black/60 focus:outline-none focus:ring-2 focus:ring-white/50"
                  ...
                  {images.length > 1 && (
                    <button
                      type="button"
                      onClick={nextImage}
                      className="absolute right-3 top-1/2 -translate-y-1/2 rounded-full bg-black/45 px-3 py-2 text-white transition-colors duration-200 hover:bg-black/60 focus:outline-none focus:ring-2 focus:ring-white/50"
