...
export default function RoomCard({ room }) {
  ...
  return (
    <>
      ...
      {/* Details Modal with Image Slider - Modern & Clean Design */}
      {showDetailsModal && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4" onClick={() => setShowDetailsModal(false)}>
          <div className="bg-white rounded-2xl w-full max-w-3xl max-h-[85vh] overflow-auto shadow-2xl animate-in zoom-in-95 duration-200" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="sticky top-0 bg-white/95 backdrop-blur-sm border-b border-gray-100 px-6 py-4 flex justify-between items-center z-10">
              ...
              <button
                ...
                className="w-8 h-8 rounded-full bg-gray-100 hover:bg-gray-200 text-gray-600 hover:text-gray-900 transition-all duration-200 flex items-center justify-center"
              >
                <i className="fas fa-times text-sm"></i>
              </button>
            </div>
            
            {/* Modal Content - Reduced padding for cleaner look */}
            <div className="px-6 py-5">
              ...
              <div className="grid grid-cols-2 gap-5 mb-6">
                ...
                <div className="bg-gray-50 rounded-xl p-3">
                  ...
                  <p className="text-base text-textPrimary flex items-center gap-2">
                    <i className="fas fa-users text-ocean-light text-sm"></i>
                    {room.capacityMin && room.capacityMax 
                      ? `${room.capacityMin}–${room.capacityMax} Guests` 
                      : room.capacity || `${room.capacityMin || room.capacityMax} Guests`}
                  </p>
                ...
                <div className="bg-gradient-to-r from-ocean-pale/30 to-ocean-ice/30 rounded-xl p-3 col-span-2">
                  <label className="block text-xs font-semibold text-ocean-mid uppercase tracking-wide mb-1">Price per Night</label>
              ...
              {/* Inclusions Section */}
              <div className="mb-5">
                <label className="block text-xs font-semibold text-gray-500 uppercase tracking-wide mb-2">Inclusions</label>
                <div className="flex flex-wrap gap-2">
                  {room.inclusions && room.inclusions.length > 0 ? (
                    room.inclusions.map((inclusion, idx) => (
                      <span key={idx} className="px-3 py-1.5 bg-ocean-ice text-ocean-mid rounded-lg text-sm font-medium">
            ...
            <div className="sticky bottom-0 bg-white/95 backdrop-blur-sm border-t border-gray-100 px-6 py-4 flex gap-3 justify-end">
              <button
                onClick={() => setShowDetailsModal(false)}
                className="px-5 py-2 border border-gray-300 rounded-lg text-gray-700 text-sm font-medium hover:bg-gray-50 transition-all duration-200"
              >
                Close
              </button>
              <button 
                onClick={handleBookNow}
                className="px-5 py-2 bg-gradient-to-r from-ocean-mid to-ocean-light rounded-lg text-white text-sm font-medium shadow-md hover:shadow-lg hover:-translate-y-0.5 transition-all duration-200"
              >
                Book Now
              </button>
            </div>
