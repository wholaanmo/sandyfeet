  ...
  return (
    <>
      <div className="bg-white rounded-2xl shadow-md overflow-hidden hover:shadow-xl transition-all duration-300 group">
        ...
        <div className="relative bg-gradient-to-br from-ocean-pale to-ocean-ice overflow-hidden" style={{ aspectRatio: '16 / 9' }}>
        ...
        <div className="p-5">
          <div className="flex justify-between items-start mb-2">
            <h3 className="text-lg font-bold text-textPrimary font-playfair leading-tight">{room.type}</h3>
            <p className="text-xl font-bold text-ocean-mid whitespace-nowrap ml-3">
          ...
          {/* Capacity */}
          <div className="flex gap-4 mt-2 text-sm text-textSecondary border-b border-ocean-light/10 pb-3">
          ...
          {/* Description Preview */}
          <p className="text-sm text-textSecondary mt-2.5 line-clamp-2">
            {room.description}
          </p>
      ...
      {showDetailsModal && (
        ...
        >
          <div
            className="bg-white rounded-2xl w-full max-w-xl max-h-[80vh] overflow-auto shadow-2xl"
            style={{ animation: 'modalIn 0.18s ease-out' }}
            onClick={(e) => e.stopPropagation()}
          >
            {/* Modal Header */}
            <div className="sticky top-0 bg-white/95 backdrop-blur-sm border-b border-gray-100 px-5 py-3.5 flex justify-between items-center z-10">
              <h2 className="text-lg font-bold text-textPrimary font-playfair">{room.type}</h2>
              <button
                onClick={() => setShowDetailsModal(false)}
            ...
            <div className="px-5 py-4">
              ...
              <div className="grid grid-cols-2 gap-3 mb-4">
                <div className="bg-gray-50 rounded-xl p-3">
                  <label className="block text-xs font-semibold text-gray-400 uppercase tracking-wide mb-0.5">Room Type</label>
                  <p className="text-sm font-semibold text-textPrimary">{room.type}</p>
                </div>
                <div className="bg-gray-50 rounded-xl p-3">
                  <label className="block text-xs font-semibold text-gray-400 uppercase tracking-wide mb-0.5">Capacity</label>
                ...
                <div className="bg-gradient-to-r from-ocean-pale/30 to-ocean-ice/30 rounded-xl p-3 col-span-2">
              ...
              {/* Inclusions */}
              <div className="mb-4">
                <label className="block text-xs font-semibold text-gray-400 uppercase tracking-wide mb-2">Inclusions</label>
                <div className="flex flex-wrap gap-1.5">
                  {room.inclusions && room.inclusions.length > 0 ? (
                    room.inclusions.map((inclusion, idx) => (
                      <span key={idx} className="px-2.5 py-1 bg-ocean-ice text-ocean-mid rounded-lg text-xs font-medium">
            ...
            {/* Modal Footer */}
            <div className="sticky bottom-0 bg-white/95 backdrop-blur-sm border-t border-gray-100 px-5 py-3.5 flex gap-2.5 justify-end">
              <button
                onClick={() => setShowDetailsModal(false)}
                className="px-4 py-1.5 border border-gray-300 rounded-lg text-gray-700 text-sm font-medium hover:bg-gray-50 transition-all duration-200"
              >
                Close
              </button>
              <button
                onClick={handleBookNow}
                className="px-4 py-1.5 bg-gradient-to-r from-ocean-mid to-ocean-light rounded-lg text-white text-sm font-medium shadow-md hover:shadow-lg hover:-translate-y-0.5 transition-all duration-200"
              >
                Book Now
              </button>
            </div>
