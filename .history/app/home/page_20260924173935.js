...
function HomePageContent() {
  ...
  const [testimonialLightbox, setTestimonialLightbox] = useState(null);
  ...
  useEffect(() => {
    try {
      if (sessionStorage.getItem('sandyfeet_ai_recommendation_return') === 'true') {
        sessionStorage.removeItem('sandyfeet_ai_recommendation_return');
        setShowAIRecommendation(true);
      }
    } catch (error) {
      console.error('Unable to restore AI recommendation view:', error);
    }
  }, []);
      ...
      <div className="bg-white min-h-screen overflow-hidden text-[#143B36]">
              ...
              {featuredRoomsLoading ? (
                ...
                featuredRooms.map((room, idx) => (
                  ...
                  <Link
                    ...
                    className="group flex h-full flex-col rounded-[2.25rem] border border-gray-100 bg-white p-4 shadow-[0_16px_40px_rgba(15,40,36,0.06)] transition-all duration-500 hover:-translate-y-1 hover:shadow-[0_18px_50px_rgba(15,40,36,0.1)] focus:outline-none focus:ring-2 focus:ring-[#3B82F6]/40"
                  >
                    ...
                    <div className="flex flex-grow flex-col px-2">
                      ...
                      <div className="mt-auto flex flex-col items-start gap-4 sm:flex-row sm:items-center sm:justify-between">
                        ...
                        <span className="rounded-full border border-blue-100 bg-blue-50/70 px-5 py-2.5 text-sm font-semibold text-blue-700 transition-all duration-300 group-hover:bg-blue-100 group-hover:shadow-sm">
                          View Details
                        </span>
                      </div>
                    </div>
                  ...
                  <div className="testimonials-track flex w-max gap-6 pb-2">
                    {(publishedFeedbacks.length <= 2
                      ...
                      return (
                        <div
                          key={`${item.id}-${index}`}
                          className="w-[360px] flex-none rounded-3xl border border-slate-100 bg-white p-8 shadow-[0_10px_35px_rgba(0,0,0,0.03)] transition-all duration-300 hover:shadow-[0_15px_45px_rgba(0,0,0,0.06)] md:w-[420px] md:p-9"
                        >
                              ...
                              {item.images.filter(Boolean).slice(0, 2).map((url, photoIndex) => (
                                ...
                                >
                                  <Image
                                    ...
                                    fill
                                    className="object-cover"
                                    sizes="(max-width: 768px) 300px, px"
                                  />
                                </button>
                              ))}
                            </div>
        ...
        {testimonialLightbox && (
          <div
            ...
            onClick={() => setTestimonialLightbox(null)}
            role="dialog"
            aria-modal="true"
          ...
          >
            ...
            >
              <button
                type="button"
                onClick={() => setTestimonialLightbox(null)}
                className="absolute -top-3 -right-3 z-10 flex h-10 w-10 items-center justify-center rounded-full bg-white text-gray-700 shadow-lg transition hover:bg-gray-100"
              ...
              <div className="overflow-hidden rounded-2xl bg-black/20 shadow-2xl">
                ...
                <img
                  src={testimonialLightbox.url}
                  alt={testimonialLightbox.alt}
