...
export default function AdminReports() {
  ...
  // Update selected year revenue when selectedYear changes
  useEffect(() => {
    if (selectedYear && yearlyMonthlyRevenueData[selectedYear]) {
      const yearTotal = yearlyMonthlyRevenueData[selectedYear].reduce((sum, month) => sum + (month.total || 0), 0);
      setSelectedYearRevenue(yearTotal);
    } else {
  ...
  const processData = (bookings, dayTours) => {
    let revenue = 0;
    let roomUnitCount = 0; // Changed from roomBookingCount to count room units
    let dayTourGuestCount = 0;
    
    const processedBookings = [];
    const multiRoomGroups = new Map();
    
    // First pass: collect completed bookings and count room units
    bookings.forEach(booking => {
      ...
      // Count room units from completed bookings only
      if (booking.status === 'completed') {
        // Count number of rooms in this booking
        let roomCount = 0;
        
        if (booking.isMultiRoomBooking && booking.parentBookingId) {
          // This is a child booking - will be handled in group processing
          ...
          // Count rooms for this child booking
          if (booking.isExclusiveResortBooking) {
            // For exclusive resort, count all room types except tents as separate rooms
            // This depends on the roomTypes array in the booking
            if (booking.roomTypes && Array.isArray(booking.roomTypes)) {
              booking.roomTypes.forEach(rt => {
                if (rt.type !== 'Tent') {
                  group.roomUnitCount += (rt.quantity || 1);
                }
              });
            }
            // Also count tents separately? Tents are additional structures
            if (booking.tentCount) {
              group.roomUnitCount += booking.tentCount;
            }
          } else {
            // Regular room booking - each booking represents one room unit
            group.roomUnitCount += (booking.numberOfRooms || 1);
          }
          ...
          if (booking.isExclusiveResortBooking) {
            ...
            if (booking.roomTypes && Array.isArray(booking.roomTypes)) {
              booking.roomTypes.forEach(rt => {
                if (rt.type !== 'Tent') {
                  roomUnitCount += (rt.quantity || 1);
                }
              });
            }
            if (booking.tentCount) {
              roomUnitCount += booking.tentCount;
            }
          } else {
            roomUnitCount += (booking.numberOfRooms || 1);
          }
    ...
    // Count day tour guests from completed status bookings
    ...
    setTotalRoomBookings(roomUnitCount);
    ...
    processedBookings.forEach(booking => {
      ...
      if (!yearlyMonthlyTrend[year]) {
        yearlyMonthlyTrend[year] = {
          roomBookings: new Array(12).fill(0), // This now counts room units
      ...
      // Count room units for trend (using numberOfRooms or child bookings count)
      let roomUnitsForMonth = 0;
      if (booking.isGrouped && booking.childBookings) {
        booking.childBookings.forEach(cb => {
          roomUnitsForMonth += (cb.numberOfRooms || 1);
        });
      } else {
        roomUnitsForMonth = (booking.numberOfRooms || 1);
      }
      yearlyMonthlyTrend[year].roomBookings[month] += roomUnitsForMonth;
    });
  ...
  const CustomTooltip = ({ active, payload, label }) => {
    if (active && payload && payload.length) {
      ...
      // Check if we're showing total combined revenue
      const isTotal = payload[0]?.dataKey === 'total';
      return (
        <div className="bg-white p-3 rounded-lg shadow-lg border border-gray-200">
          ...
          {payload.map((entry, index) => {
            if (entry.dataKey === 'total') {
              return (
                <p key={index} className="text-sm font-bold text-[#1E3A8A]">
                  Total Revenue: ₱{entry.value.toLocaleString()}
                </p>
              );
            }
            return (
  ...
  return (
    <div className="px-9 py-1 min-h-screen" style={{ backgroundColor: 'var(--color-blue-whites)' }}>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-10">
        ...
        <div className="group bg-gradient-to-br from-white via-white to-emerald-50/30 rounded-2xl shadow-md border border-[#10B981]/20 overflow-hidden hover:shadow-xl hover:border-[#10B981]/40 transition-all duration-300">
          <div className="p-6">
            ...
            <p className="text-xs text-[#1E3A8A]/50">Total room units from <strong>completed</strong> reservations</p>
          </div>
        </div>
      ...
      {/* Tabs */}
      <div className="relative flex gap-2 mb-6 border-b border-[#4D8CF5]/20 overflow-x-auto" ref={tabsContainerRef}>
        {/* Sliding background */}
        <div
          ref={sliderRef}
      ...
      <div className="bg-white rounded-2xl shadow-lg border border-gray-200 p-6 md:p-8">
        ...
        {activeTab === 'revenue' && (
          <div className="animate-fadeIn">
            ...
            {selectedYear && selectedYearRevenue > 0 && (
              <div className="mb-6 p-4 bg-gradient-to-r from-[#4D8CF5]/10 to-[#7AAAF8]/10 rounded-xl border border-[#4D8CF5]/20">
                <div className="flex items-center justify-between">
                  <div>
                    <p className="text-sm text-[#1E3A8A]/70">Total Revenue for {selectedYear}</p>
                    <p className="text-2xl font-bold text-[#1E3A8A]">₱{selectedYearRevenue.toLocaleString()}</p>
                  </div>
                  <div className="w-10 h-10 rounded-full bg-[#4D8CF5]/20 flex items-center justify-center">
                    <i className="fas fa-chart-line text-[#4D8CF5] text-lg"></i>
                  </div>
                </div>
              </div>
            )}
              ...
              <div className="bg-gradient-to-br from-gray-50 to-white rounded-xl p-6 border border-gray-100 shadow-sm hover:shadow-md transition-shadow duration-300">
                <div className="flex items-center gap-3 mb-5 pb-3 border-b border-gray-200">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-gray-500">Number of room units over time (completed bookings)</p>
                  </div>
                </div>
                {selectedYear && yearlyMonthlyTrendData[selectedYear] && (
                  <ResponsiveContainer width="100%" height={320}>
                    <LineChart data={yearlyMonthlyTrendData[selectedYear]} margin={{ top: 30, right: 30, left: 20, bottom: 20 }}>
                      ...
                      <Line type="monotone" dataKey="roomBookings" name="Room Units Booked" stroke="#4D8CF5" strokeWidth={3} dot={{ r: 5, fill: '#4D8CF5', strokeWidth: 2 }} activeDot={{ r: 8 }} />
                    </LineChart>
                  </ResponsiveContainer>
                )}
              </div>
              <div className="bg-gradient-to-br from-gray-50 to-white rounded-xl p-6 border border-gray-100 shadow-sm hover:shadow-md transition-shadow duration-300">
                <div className="flex items-center gap-3 mb-5 pb-3 border-b border-gray-200">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-gray-500">Number of guests on day tours (completed bookings)</p>
                  </div>
                </div>
                {selectedYear && yearlyMonthlyTrendData[selectedYear] && (
                  <ResponsiveContainer width="100%" height={320}>
