...
export default function AdminReports() {
  ...
  // Initialize slider after mount and ensure active tab is visible
  useEffect(() => {
  ...
  const processData = (bookings, dayTours) => {
    ...
    processedBookings.forEach(booking => {
      ...
      // Count room units for trend
      let roomUnitsForMonth = 0;
      if (booking.isGrouped && booking.childBookings) {
        booking.childBookings.forEach(cb => {
          if (cb.isExclusiveResortBooking) {
            if (cb.roomTypes && Array.isArray(cb.roomTypes)) {
              cb.roomTypes.forEach(rt => {
                if (rt.type !== 'Tent') {
                  roomUnitsForMonth += (rt.quantity || 1);
                }
              });
            }
            if (cb.tentCount) {
              roomUnitsForMonth += cb.tentCount;
            }
          } else {
            roomUnitsForMonth += (cb.numberOfRooms || 1);
          }
        });
      } else if (booking.isExclusiveResortBooking) {
        if (booking.roomTypes && Array.isArray(booking.roomTypes)) {
          booking.roomTypes.forEach(rt => {
            if (rt.type !== 'Tent') {
              roomUnitsForMonth += (rt.quantity || 1);
        ...
        if (booking.tentCount) {
          roomUnitsForMonth += booking.tentCount;
        }
      } else {
  ...
  const CustomTooltip = ({ active, payload, label }) => {
    if (active && payload && payload.length) {
      // Check if we're in revenue tab to show combined total
      ...
      return (
        <div className="bg-white p-3 rounded-lg shadow-lg border border-gray-200">
          ...
          {payload.map((entry, index) => {
            // For revenue tab, show individual breakdown plus combined total
            if (isRevenueTab && (entry.dataKey === 'roomRevenue' || entry.dataKey === 'dayTourRevenue')) {
              ...
              return (
                <div key={index}>
                  ...
                  <p className="text-sm font-bold text-[#1E3A8A] mt-1 pt-1 border-t border-gray-200">
                    Combined Total: ₱{totalRevenue.toLocaleString()}
                  </p>
                </div>
              );
            }
  ...
  return (
    <div className="px-9 py-1 min-h-screen" style={{ backgroundColor: 'var(--color-blue-whites)' }}>
      ...
      <div className="relative flex w-full mb-6 border-b border-[#4D8CF5]/20" ref={tabsContainerRef}>
        {/* Sliding background */}
        <div
          ref={sliderRef}
          className="absolute bottom-0 h-0.5 bg-gradient-to-r from-[#4D8CF5] to-[#7AAAF8] transition-all duration-300 ease-in-out rounded-full"
