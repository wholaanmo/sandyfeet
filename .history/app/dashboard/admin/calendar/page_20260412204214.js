...
export default function AdminCalendar() {
  ...
  const getDateStatus = (date) => {
    ...
    if (
      blockInfo.status === 'partiallyBlocked' &&
      blockInfo.morningClosed &&
      !blockInfo.afternoonClosed &&
      isDateFullyBookedByGuests(date)
    ) {
      return 'checkOutRestrictedFullyBooked';
    }
          ...
          <div className="bg-white rounded-2xl shadow-lg overflow-hidden h-full">
            ...
            <div className="p-6">
                ...
                {days.map((day, idx) => {
                  ...
                  switch(status) {
                    case 'past':
                      ...
                      titleText = 'Past date';
                      break;
                    case 'checkOutRestrictedFullyBooked':
                      bgColor = 'bg-white';
                    ...
                    case 'fullyBooked':
                      ...
                      cursorClass = 'cursor-not-allowed';
                      titleText = 'Fully booked by guests (no check-out restriction)';
                      break;
                    case 'checkInBlocked':
                  ...
                  const isDualCheckoutBooked = status === 'checkOutRestrictedFullyBooked';
                  ...
                  return (
                    <button
                      ...
                      onClick={() =>
                        status !== 'past' &&
                        status !== 'fullyBooked' &&
                        status !== 'checkOutRestrictedFullyBooked' &&
                        status !== 'fullyBlocked' &&
                    ...
                    >
                      <span
                        className={`absolute inset-0 flex items-center justify-center text-sm font-medium ${textColor} ${
                          isDualCheckoutBooked ? 'pb-2.5' : ''
                        }`}
                      >
                        {day.getDate()}
                      </span>
                      {isDualCheckoutBooked && (
                        <span
              ...
              <div className="mt-6 pt-4 border-t border-ocean-light/10 flex justify-center gap-6 text-xs flex-wrap">
                ...
                <div className="flex items-center gap-1.5"><div className="w-3 h-3 bg-red-100 border border-red-200 rounded"></div><span>Fully booked (no check-out restriction)</span></div>
                ...
                <div className="relative w-3 h-3 bg-white border border-white rounded flex items-end justify-center pb-0.5 gap-0.5">
                    <span className="w-10 h-1.5 rounded-full bg-green-400" />
                    <span className="w-10 h-1.5 rounded-full bg-red-500" />
                  </div>
                  <span>Fully booked + check-out restricted (dots below date)</span>
                </div>
                ...
                <div className="flex items-center gap-1.5"><div className="w-3 h-3 bg-orange-100 border border-orange-200 rounded"></div><span>Not Available</span></div>
